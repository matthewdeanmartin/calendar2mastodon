# Security Policy for `calendar2mastodon`
...
1. **GitHub private vulnerability reporting** —
   [submit a confidential report](https://github.com/matthewdeanmartin/calendar2mastodon/security/advisories/new)
...
For maximum isolation, run `calendar2mastodon` inside a container so it only has access to what you
explicitly mount.
...
```bash
docker build -t calendar2mastodon .

# Mount the current directory into /data and pass it as an argument
docker run --rm -v "$(pwd):/data" calendar2mastodon /data
```
